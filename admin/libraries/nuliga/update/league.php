...
class NuLigaUpdateLeague
{
    ...
    /**
     * Stores the current NuLiga league teams with the database.
     *
    ...
    public function update($tableId, $leagueteams)
    {
        ...
        {
            if (!self::isLeagueteamExisting($leagueteam['name']))
            {
     ...
     */
    protected static function isLeagueteamExisting($name)
    {
        $db    = JFactory::getDbo();
        $query = $db->getQuery(true);

        $query->select('COUNT(*)')
            ->from('#__nuliga_leagueteams')
